...
class PostDetail(RatelimitMixin, DetailView):
    ...
    def get_queryset(self):
        queryset = (
            super(PostDetail, self).get_queryset().filter(pub_date__lte=timezone.now())
        )
        return queryset
    ...
    def get_object(self, *args, **kwargs):
        ...
        if key:
            post_object = get_object_or_404(
                self.model, slug=self.kwargs["slug"], lock_key=key
            )
        else:
...
class PostList(RatelimitMixin, ListView):
    ...
    def get_queryset(self):
        queryset = self.model.objects.filter(pub_date__lte=timezone.now()).order_by(
            self.ordering
        )
        return queryset
...
class PostListByTag(PostList):
    def get_queryset(self):
        queryset = (
            super(PostListByTag, self)
            .get_queryset()
            .filter(keywords__icontains=self.kwargs["keyword"])
        )
        return queryset
