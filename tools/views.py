...
def remove_metadata(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            cleaned_filename = filename.with_name(filename.name.replace(exts, ".cleaned" + exts))
            ...
            try:
                return FileResponse(tmp_file_path.open("rb"), as_attachment=True, filename=cleaned_filename.name)
            finally:
